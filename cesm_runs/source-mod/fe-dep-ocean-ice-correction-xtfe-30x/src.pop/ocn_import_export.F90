module ocn_import_export
   ...
   use forcing_fields,    only: ATM_XTFE_FLUX, SEAICE_XTFE_FLUX
...
!-----------------------------------------------------------------------
      ...
      do i=this_block%ib,this_block%ie
         ...
         SEAICE_XTFE_FLUX(i,j,iblock) = 30.0_r8 * 0.1_r8 * RCALCT(i,j,iblock) * ( &
           x2o(index_x2o_Fioi_xtFe_ice_rxn,n))

         ! convert Fe flux from MKS (kg/m^2/s) to CGS (g/cm^2/s)
         ATM_XTFE_FLUX(i,j,iblock) = 30.0_r8 * 0.1_r8 * RCALCT(i,j,iblock) * ( &
